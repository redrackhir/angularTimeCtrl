...
export class CompanyService {
  ...
  getCompaniesList(): Observable<Company[]> {
    return this.http.get<Company[]>(this.PHP_API_SERVER + '/api/readCompanies.php')
      .pipe(
        tap(_ => this.log('fetched Companyes')),
    ...
    getCompanyNo404<Data>(id: number): Observable<Company> {
      ...
      return this.http.get<Company[]>(url)
        .pipe(
          map(Companyes => Companyes[0]), // returns a {0|1} element array
          tap(h => {
    ...
    updateCompany(company: Company): Observable<any> {
      return this.http.put(`${this.PHP_API_SERVER}/updateCompany.php?id=${company.codigoEmpresa}`, company, this.httpOptions).pipe(
